...
async function base64Encode(file) {
  const image = await sharp(file).resize(600, 600).toFormat('jpeg').jpeg({quality: 70, force: true}).toBuffer();
    ...
    feed: [
            ...
            feed.addItem({
              ...
              content: `
                <p>
                  <img
                    alt="Cover image"
                    src="${await base64Encode(`./assets/media-uploads/${post.img}`)}"
                  >
                </p>
                ${post.bodyHtml}
              `,
      ...
      },
      {
        create: async feed => {
          ...
          const posts = await $content('help')
            .sortBy('title', 'desc')
            .fetch()
          posts.forEach(post => {
